...
import com.example.sisvitafrontend.ui.components.CustomDialog
import com.example.sisvitafrontend.ui.components.CustomEditTextField
import com.example.sisvitafrontend.ui.components.CustomFooter
import com.example.sisvitafrontend.ui.components.CustomTextField
import com.example.sisvitafrontend.ui.components.DatePickerComponent
...
private fun Formulary3(
    ...
    val documentType by registerViewModel.documentType.observeAsState(-1)
    ...
    SelectList(label = R.string.document_type,
        selectedItem = if (documentType == -1) "" else documentList[documentType].type,
        items = documentList.map { it.type },
        onItemSelected = {
            registerViewModel.onDocumentTypeChanged(
                documentList.find { document -> document.type == it }?.id?.minus(1) ?: -1
            )
        })
    Spacer(modifier = Modifier.padding(bottom = 10.dp))
    CustomTextField(label = R.string.document,
        ...
        pattern = { it ->
            val documentLength = documentList.find { it.id.minus(1) == documentType }?.length ?: 0
...
private fun Formulary4(
    ...
    val sex by registerViewModel.gender.observeAsState(-1)
    val phone by registerViewModel.phone.observeAsState("")
    val email by registerViewModel.email.observeAsState("")
    val sexList by registerViewModel.genderList.observeAsState(emptyList())

    SelectList(label = R.string.sex,
        selectedItem = if (sex == -1) "" else sexList[sex].gender,
        items = sexList.map { it.gender },
        onItemSelected = {
            registerViewModel.onGenderChanged(
                sexList.find { sex -> sex.gender == it }?.id?.minus(1) ?: -1
            )
        })
    Spacer(modifier = Modifier.padding(bottom = 10.dp))
    CustomEditTextField(
